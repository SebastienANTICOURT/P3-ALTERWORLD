...
function FiltresBar({}) {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {console.log(res.data); setUnivers(res.data);})
    axios
      ...
      <div className="FB">
        <div className="DivFB">
          <h1>Univers</h1>
           ...
           <p 
           key={univer.id}
           className={selectedUnivers.includes(univer.id) ? "selected" : "defaultColor"}
           onClick={() => {
               if (selectedUnivers.includes(univer.id)) {
                   setSelectedUnivers(selectedUnivers.filter(id => id !== univer.id));
               } else {
        ...
        <div className="DivFB">
          <h1>Objets</h1>
            {natures.map((nature) =>(
             <p 
             key={nature.id}
             className={selectedNatures.includes(nature.id) ? "selected" : "defaultColor"}
             onClick={() => {
                 if (selectedNatures.includes(nature.id)) {
                     setSelectedNatures(selectedNatures.filter(id => id !== nature.id));
                 } else {
